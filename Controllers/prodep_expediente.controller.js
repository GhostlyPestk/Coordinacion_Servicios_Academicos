...
module.exports.subirExpediente = catchAsync(async (req, res, next) => {
  ...
  var laboral =
    resp.laboral.contratos_academicos[
      resp.laboral.contratos_academicos.length - 1
    ];
  if (resp.status === 'activo') {
    // && laboral.temporalidad === "tiempo completo"
    const dir = `./Files/uploads/${req.params.codigo}/expediente/`;
    const expedienteDigitalizado = req.file.originalname;
    const fileType = req.file.mimetype;
    await fs.mkdir(dir, { recursive: true }, err => {
      if (err) {
        return next(new AppError('Error al crear la carpeta', 500));
      }
    });
    await fs.rename(
      `./Files/uploads/${expedienteDigitalizado}`,
      dir + expedienteDigitalizado,
      err => {
        if (err) {
          return next(
    ...
    if (!buffer) {
      return next(new AppError('Error al leer el documento', 500));
    }
    ...
    if (!PROD) {
      return next(new AppError('Error al guardar expediente', 500));
    }
    ...
    return next(
      new AppError(
        'El profesor no es de tiempo completo o no esta activo en SIIPERSU',
        400
      )
    );
  }
...
module.exports.actualizarExpediente = catchAsync(async (req, res, next) => {
  ...
  if (!resp) {
    return next(new AppError('El codigo no esta en la base de datos', 400));
  }
  var laboral =
    resp.laboral.contratos_academicos[
      resp.laboral.contratos_academicos.length - 1
  ...
  if (resp.status === 'activo' && laboral.temporalidad === 'tiempo completo') {
    ...
    const PROD = await resp.save();
    if (!PROD) {
      return next(new AppError('Error al actualizar expediente', 500));
    }
    const prod = resp.prodep[last];
    res.status(200).json({ ok: true, prod });
  } else {
...
module.exports.descargarExpediente = catchAsync(async (req, res, next) => {
  ...
  await fs.writeFile(dir, buffer, err => {
    if (err) {
      return next(new AppError('Error al escribir el archivo', 500));
    }
  });
...
module.exports.verExpediente = catchAsync(async (req, res, next) => {
  ...
  if (!exp) {
    return next(new AppError('El archivo no esta en la base de datos', 400));
  }
