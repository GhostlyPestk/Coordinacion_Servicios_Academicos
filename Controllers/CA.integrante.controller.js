...
module.exports.postCuerpoAcademicoIntegrante = catchAsync(
  async (req, res, next) => {
    console.log(req.body.codigo);
    const trabajador = await Trabajador.findOne({ codigo: req.body.codigo });
    if (!trabajador) {
      return next(new AppError('No se existe el trabajador', 400));
    }
    var integrante = { integrante: trabajador._id, tipo: req.body.tipo };
    const inscrito = await CuerpoAcademico.findOne({
      integrantes: { $in: trabajador._id }
    });
    if (inscrito) {
      return next(
        new AppError(
          'El trabajador ya esta inscrito en un Cuerpo Academico',
          400
        )
      );
    }
    ...
    if (!resp) {
      return next(new AppError('Error al guardar el documento', 500));
    }
    res.status(200).json({ ok: true, resp });
  }
);

module.exports.deleteCuerpoAcademicoIntegrante = catchAsync(
  async (req, res, next) => {
    const trabajador = await Trabajador.findOne({ codigo: req.body.codigo });
    if (!trabajador) {
      return next(new AppError('No se existe el trabajador', 400));
    ...
    const CA = await CuerpoAcademico.findOne({ clave: req.params.id });
    if (!CA) {
      return next(new AppError('No se encuentra el docmuento en la BD', 400));
    }
